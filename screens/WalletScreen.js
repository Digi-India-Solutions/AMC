...
export default function WalletManagement({ navigation }) {
  ...
  const closeDrawer = () => {
    Animated.timing(slideAnim, { toValue: -width * 0.75, duration: 250, useNativeDriver: true }).start(() =>
      setShowDrawer(false)
    );
  };
  ...
  const filteredData =
    selectedTab === "Wallet Balances"
      ? wallet.users.filter((u) => u.name.toLowerCase().includes(search.toLowerCase()))
      : wallet.transactions.filter((t) => t.name.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
      <Animated.View style={[styles.userTransactionCard, { opacity: anim, transform: [{ translateY }] }]}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          <Text style={[styles.amount, { color: "#1E88E5" }]}>₹{item.balance.toLocaleString()}</Text>
          <Text style={{ marginTop: 4, color: item.status === "Active" ? "#00C853" : "#D32F2F", fontWeight: "600", fontSize: 12 }}>
            {item.status}
          </Text>
        </View>
      </Animated.View>
      ...
      >
        ...
        {selectedTab === "Wallet Balances" ? (
          filteredData.length > 0 ? (
            <FlatList data={filteredData} keyExtractor={(item) => item.id.toString()} renderItem={renderUserCard} showsVerticalScrollIndicator={false} />
          ) : (
            <Text style={styles.noData}>No users found.</Text>
          )
        ) : (
          <FlatList data={filteredData} keyExtractor={(item) => item.id.toString()} renderItem={renderTransaction} showsVerticalScrollIndicator={false} />
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  transactionCard: { backgroundColor: "#fff", padding: 16, borderRadius: 14, marginBottom: 12, elevation: 3, shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 3 },
  ...
  userTransactionCard: { backgroundColor: "#fff", padding: 16, borderRadius: 14, marginBottom: 12, elevation: 3, flexDirection: "row", alignItems: "center", shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 3 },
